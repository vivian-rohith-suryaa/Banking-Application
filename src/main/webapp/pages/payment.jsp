...
%>
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/styles/payment.css">

<div class="payment-wrapper">
	<div class="payment-container">
		<h2>Payment</h2>
		<form id="payment-form" class="glass">
			<label> Account ID: <select name="accountId"
				id="account-select" required>
					<option value="">-- Select Account --</option>
			</select>
			</label> <label> Payment Mode: <select name="paymentMode"
				id="payment-mode" required>
			...
			<div id="bank-transfer-options" class="hidden">
				<label for="isExternal" style="margin-right: 10px;">External
					Transfer:</label>
				<div id="transfer-type" style="display: flex; gap: 20px;">
					<label><input type="radio" name="isExternal" value="true">
						Yes</label> <label><input type="radio" name="isExternal"
						value="false" checked> No</label>
				</div>
			</div>

			<div id="to-account-group" class="hidden">
				<label id="to-account-label"> To Account ID: <select
					name="transactedAccount" id="to-account-select" class="hidden">
						<option value="">-- Select Account --</option>
				</select> <input type="text" name="transactedAccount" id="to-account-input"
					maxlength="12" class="hidden"
			...
			<div id="ifsc-group" class="conditional hidden">
				<label> IFSC Code: <input type="text"
					name="externalIfscCode" id="ifsc-code"
					placeholder="Enter target IFSC" maxlength="20"
			...
			<label> Amount: <input type="text" name="amount"
				maxlength="7" required placeholder="Maximum Input Limit - 7 digits"
			...
			<div class="modal-card">
				...
				<form id="pin-confirm-form">
					<input type="password" name="pin" required maxlength="6"
						placeholder="Enter 6-digit PIN" pattern="\d{6}"
						oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
