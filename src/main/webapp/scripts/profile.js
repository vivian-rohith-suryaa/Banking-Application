export function initProfilePage(contextPath, userId, userRole) {
	console.log("Initializing profile for user:", userId, userRole);
	...
	fetch(`${contextPath}/viiva/user/${userId}`, {
		...
		.then(res => {
			...
			Object.entries(fields).forEach(([key, el]) => {
				...
				if (el) el.textContent = originalData[key];
			});
		})
		.catch(err => {
	...
	document.getElementById("edit-profile-btn")?.addEventListener("click", () => {
		...
		Object.entries(fields).forEach(([key, el]) => {
			const editableForUser = userRole === 1 && ["email", "phone", "address"].includes(key);
			const editableForOthers = userRole > 1 && ["email", "phone"].includes(key);

			if ((editableForUser || editableForOthers) && el) {
	...
	saveBtn?.addEventListener("click", () => {
		...
		if (!validateProfilePayload(payload, userRole)) return;
...
function validateProfilePayload({ user, customer }, userRole) {
	const messages = [];
...
/*export function initProfilePage(contextPath, userId, userRole) {
	...
	document.getElementById("edit-profile-btn")?.addEventListener("click", () => {
		...
		Object.entries(fields).forEach(([key, el]) => {
			const editableForCustomer = userRole < 2 && !["email", "phone","address"].includes(key);
			const editableForEmployee = userRole >= 2 && ["email", "phone"].includes(key);

			if (editableForCustomer || editableForEmployee) {
				el.contentEditable = "true";
				el.classList.add("editable");
			}
		});
